...
const config = require('./images.config.json')
const args = process.argv
const shouldRename = args.slice(2).includes('rename')

const { sizes, thumbBlurLarge, thumbBlurSmall, location: imageLocation, quality, jsPrefix } = config
...
const manifestTemplate = (name, size, file) => `export { default as ${jsPrefix || 'Img'}${name} } from './processed/${size}/${file}'`
...
async function createImageIncrements ({ file, manifest }) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve(`${file} - increments created`)
    } catch (err) {
      reject(err)
    }
  })
